...
Deno.serve(async (req) => {
  try {
    ...
    if (associate_id) {
      // Usa service role para buscar pelo ID passado (funciona independente da sessão)
      const associates = await base44.asServiceRole.entities.Associate.filter({ id: associate_id }, '', 1);
      if (!associates || associates.length === 0) {
        return Response.json({ error: 'Associate not found' }, { status: 404 });
      }
      associateData = associates[0];
    } else {
      // Fallback: tenta buscar pelo usuário autenticado
      const user = await base44.auth.me();
      if (!user) {
        return Response.json({ error: 'Unauthorized' }, { status: 401 });
      }
      const associates = await base44.asServiceRole.entities.Associate.filter({ user_id: user.id }, '', 1);
      if (!associates || associates.length === 0) {
        return Response.json({ error: 'Associate not found' }, { status: 404 });
      }
      associateData = associates[0];
    ...
    const click = await base44.asServiceRole.entities.ExternalLinkClick.create({
