      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input className="mt-1.5 font-mono" placeholder="BR123456789BR" value={form.tracking_code}
              onChange={e => setForm(f => ({ ...f, tracking_code: e.target.value }))} />
          </div>

          <div>
            <Label>Observações de Entrega</Label>
            <Input className="mt-1.5" placeholder="Ex: Entregar apenas com destinatário" value={form.delivery_notes}
          ...
          {order.shipping_city && (
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-wide mb-1 flex items-center gap-1.5">
                <Package size={11} /> Entregar em
              </p>
              <p className="text-sm text-foreground">
                {[order.shipping_street, order.shipping_number].filter(Boolean).join(', ')}
              </p>
              <p className="text-sm text-muted-foreground">
                {[order.shipping_neighborhood, order.shipping_city, order.shipping_state].filter(Boolean).join(' / ')}
